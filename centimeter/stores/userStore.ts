...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    async function logIn(email: string, password: string) {
      const { ok, status, data } = await apiCall<User | Record<string, unknown> | string | string[]>(`${backendBaseUrl()}/users/login/`, {
        method: "POST",
    ...
    async function signUp(email: string, password: string, fullName?: string) {
      ...
      const { ok, status, data } = await apiCall<User | Record<string, unknown> | string | string[]>(`${backendBaseUrl()}/users/register/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    async function listPresentations(): Promise<Presentation[]> {
      const { ok, data } = await apiCall<Presentation[]>(`${backendBaseUrl()}/presentations/list/`, { method: "GET", headers: authHeaders() });
    ...
    async function getPresentation(code: string): Promise<Presentation | null> {
      ...
      const { ok, data } = await apiCall<Presentation>(`${backendBaseUrl()}/presentations/${code}/get/`, { method: "GET", headers: authHeaders() });
    ...
    async function savePresentation(presentationData: Partial<Presentation>) {
      ...
      const { ok, data } = await apiCall<Presentation>(`${backendBaseUrl()}/presentations/${code}/save/`, {
        method: "POST",
        headers: authHeaders(),
      ...
      const existingIndex = presentations.value.findIndex((p) => p.presentation_code === code);
