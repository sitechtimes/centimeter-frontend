<template>
    <div :style="componentStyle" :class="['group select-none absolute', { 'ring-2 ring-blue-500': isSelected }]" @click.stop="emit('select')">
        <!-- Shape rendering -->
        <div :style="shapeStyle" class="absolute inset-0" />
...
const shapeStyle = computed(() => {
    const shape = props.component.content || 'rectangle'
    const bg = props.component.backgroundColor || '#4F46E5'
    const base: Record<string, string> = { backgroundColor: bg }
    ...
    if (shape === 'circle') {
        ...
        base.width = '0'
        base.height = '0'
        base.borderLeft = '50px solid transparent'
        base.borderRight = '50px solid transparent'
        base.borderBottom = `100px solid ${bg}`
        // Use percentages for proper scaling
        base.borderLeftWidth = '50%'
        base.borderRightWidth = '50%'
        base.borderBottomWidth = '100%'
    }
