<template>
  <div :style="componentStyle" :class="['group select-none absolute', { 'ring-2 ring-blue-500': isSelected }]" @click.stop="emit('select')">
    ...
    <div v-else class="absolute inset-0 flex flex-col items-center justify-center bg-[var(--faded-bg-color-light)] border-2 border-dashed border-[var(--faded-bg-color-dark)] rounded">
      ...
      <span class="text-xs text-[var(--faded-text-color)]">No image</span>
    </div>

    <div :class="['absolute inset-0 cursor-move z-10', { 'bg-blue-50 bg-opacity-10': isSelected }]" @mousedown="startDrag" />

    <div v-if="isSelected" class="absolute inset-0 pointer-events-none">
      <div
        v-for="(classes, direction) in HANDLE_MAP"
        :class="classes"
...
const startResize = (e: MouseEvent, handle: string) => {
  ...
  const onMove = (me: MouseEvent) => {
    ...
    if (handle.includes("w")) {
      w = Math.max(5, init.width - dx);
      w = Math.min(w, init.x + init.width);
      x = init.x + (init.width - w);
      if (x < 0) {
        w += x;
        x = 0;
      }
    ...
    if (handle.includes("n")) {
      ht = Math.max(5, init.height - dy);
      ht = Math.min(ht, init.y + init.height);
      y = init.y + (init.height - ht);
      if (y < 0) {
        ht += y;
        y = 0;
      }
    ...
    const dx2 = x - props.component.x,
      dy2 = y - props.component.y;
