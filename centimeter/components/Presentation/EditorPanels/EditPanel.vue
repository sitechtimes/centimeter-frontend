  ...
  <div class="relative w-full max-w-xs bg-[var(--faded-bg-color-light)] rounded-lg shadow-sm h-full flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto px-5 py-6">
      <div class="space-y-3 pb-6">
        ...
        <button @click="$emit('add-component', 'text')" class="w-full flex items-center gap-3 px-4 py-3 bg-[var(--primary)] hover:bg-[var(--primary-shade)] text-white rounded-lg transition-colors">
          <Type class="w-5 h-5" />
      ...
      <div class="space-y-3 relative py-6">
        ...
        <div class="relative">
          <button @click.stop="showTypeDropdown = !showTypeDropdown" class="w-full flex items-center gap-3 px-4 py-3 bg-[var(--bg-color)] hover:bg-neutral-200/70 rounded-xl transition-colors">
          ...
          <div v-if="showTypeDropdown" class="absolute z-50 mt-1.5 w-full bg-[var(--bg-color)] border border-neutral-200 rounded-xl shadow-lg shadow-neutral-200/50 max-h-72 overflow-y-auto">
            ...
            <button
              v-for="opt in interactiveTypes"
              :key="opt.label"
              @click="selectSlideType(opt.label)"
              :class="[
                'w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-[var(--faded-bg-color-light)] transition-colors',
      ...
      <div class="space-y-3 pt-6 mb-5" v-if="!imageOnSlide">
        <div>
          ...
          <p class="text-sm text-neutral-400 leading-relaxed">We support png, gif, jpg, jpeg, svg, webp, avif, heic and heif.</p>
        </div>
        <div
          :class="[
        ...
        >
          <p class="text-sm text-neutral-400">
            Drag and drop or
            <button class="text-[var(--primary)] hover:text-[var(--primary-shade)] font-medium underline underline-offset-2" @click="openFilePicker">Click to add image</button>
          </p>
      ...
      <div class="space-y-3 pt-6" v-if="imageOnSlide">
        <div>
          <h3 class="text-sm font-semibold text-[var(--text-color)] mb-1">Image</h3>
          <p class="text-sm text-neutral-400 leading-relaxed">We support png, gif, jpg, jpeg, svg, webp, avif, heic and heif.</p>
          <div class="my-12 flex items-center gap-4 rounded-lg">
            <div class="w-14 h-14 rounded-lg overflow-hidden border border-[var(--primary-shade-translucent)] flex-shrink-0">
      ...
      <div class="space-y-4">
        ...
        <button class="w-full text-center text-sm text-[var(--faded-text-color)] hover:text-[var(--text-color)] transition-colors py-2">Reset to theme defaults</button>
      </div>
    </div>
...
const imageOnSlide = computed(() => props.selectedSlide?.components?.some((c) => c.type === "image") ?? false);

const firstImageSrc = computed(() => props.selectedSlide?.components?.find((c) => c.type === "image")?.src ?? "");
...
const isImageFile = (file: File) => ["image/png", "image/gif", "image/jpeg", "image/jpg", "image/svg+xml"].includes(file.type);
