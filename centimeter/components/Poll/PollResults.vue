<template>
  <div class="max-w-4xl mx-auto p-6 bg-[var(--faded-bg-color-light)] rounded-xl shadow-lg">
    ...
    <p class="text-lg text-[var(--faded-text-color)] mb-6">Total Votes: {{ totalVotes }}</p>
    ...
    <div class="space-y-4">
      <div v-for="option in poll.options" :key="option.id" class="relative">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-lg font-semibold text-[var(--text-color)]"> {{ getVotes(option.id) }} ({{ getPercentage(option.id) }}%) </span>
        </div>

        <div class="w-full bg-[var(--bg-color)] rounded-full h-8 overflow-hidden">
          <div class="bg-[var(--primary)] h-full rounded-full transition-all duration-500 flex items-center justify-end pr-3" :style="{ width: `${getPercentage(option.id)}%` }">
            <span v-if="getPercentage(option.id) > 10" class="text-sm font-medium text-[var(--text-color-contrast)]"> {{ getPercentage(option.id) }}% </span>
          </div>
        </div>
      </div>
    </div>
