<template>
  <div class="flex flex-col items-center justify-center w-screen m-0">
    <header class="transition duration-500 bg-[color:var(--bg-color)] w-screen">
      <div
        class="w-screen flex items-center justify-evenly border-b-2 border-solid border-[color:var(--faded-bg-color)] bg-[color:var(--bg-color)] relative z-20"
      >
        <RouterLink
          class="flex items-center justify-center gap-2 no-underline text-[color:var(--text-color)]"
          to="/"
        >
        </RouterLink>
        <button
        ...
        >
          <img class="h-6 transition-all duration-500" :src="'/ui/arrow.svg'" aria-hidden="true" />
        </button>
        <input
          v-model="presentationName"
          class="h-16 border-solid border-2 border-[color:var(--text-color)]"
        />
        <nav class="flex items-center justify-center gap-3">
          <div class="relative group" v-for="button in presentButtons" :key="button.name">
            <RouterLink
              :to="button.path"
              class="navButton relative no-underline text-[var(--text-color)] font-bold flex items-center justify-center px-4 py-2 rounded hover:bg-[color:var(--primary-light)] transition"
            >
              {{ button.name }}
            </RouterLink>
          </div>
