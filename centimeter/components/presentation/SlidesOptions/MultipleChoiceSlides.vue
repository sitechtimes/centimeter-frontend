        ...
        <div class="flex-1 min-h-0" :class="chartLayoutClass">
          ...
          <div :class="optionsLayoutClass">
            <div
              ...
              :key="choice.backendId ?? choice.position"
              class="flex items-center gap-2 bg-[var(--bg-color)] rounded border border-[var(--faded-bg-color)] min-w-0"
              :class="[
                choiceClass,
                choice.chosen && isPresentationMode ? 'bg-[var(--primary-shade-translucent)] border-[var(--primary)] cursor-pointer' : '',
                isPresentationMode && hasVoted && !choice.chosen ? 'opacity-50' : ''
              ]"
              @click="isPresentationMode ? chooseChoice(choice) : null"
            >
              <span v-if="isPresentationMode" class="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold" :style="{ backgroundColor: choice.color, color: '#fff' }">
                {{ choice.position }}
              </span>
              ...
              <input
                ...
                :class="['w-full min-w-0 truncate bg-transparent outline-none', fontSize, isPresentationMode ? 'cursor-pointer select-none' : '']"
              />

              <span v-if="isPresentationMode && isHost" class="flex-shrink-0 text-xs text-[var(--text-color)] opacity-60 font-mono">
                {{ choice.amount_chosen ?? 0 }}
              </span>

              <button v-if="!isPresentationMode" @click="removeOption(choice)" class="text-[var(--danger)] flex-shrink-0">X</button>
            </div>

            <div v-if="isPresentationMode && hasVoted" class="mt-2 text-sm text-[var(--primary)] font-semibold">✓ Vote submitted</div>
...
async function chooseChoice(choice: PollsOption) {
  ...
  if (responseLimit === 1) {
    ...
    try {
      await responseStore.votePolls(props.activePollId!, props.sessionJoinCode, props.nickname, choice.backendId!);
    ...
    if (choice.chosen) {
      ...
      if (currentlyChosen.length + 1 === responseLimit) {
        ...
        try {
          await responseStore.voteMultiSelect(props.activePollId!, props.sessionJoinCode, props.nickname, chosenIds);
          setHasVotedForCurrentSlide(true);
        } catch (err) {
...
const rootClass = computed(() => {
  return isPresentationMode.value ? "w-full h-full flex items-center justify-center" : "flex-1 flex items-center justify-center p-4 bg-[var(--bg-color)]";
});
...
const chartLayoutClass = computed(() => {
  return chartType.value === "bar" ? "grid h-full grid-rows-[1.4fr_1fr] gap-4" : "grid h-full grid-cols-[1.1fr_1fr] gap-4 items-center";
});

const chartShellClass = computed(() => {
  return chartType.value === "bar" ? "min-h-0 rounded-lg border border-[var(--faded-bg-color)] p-2" : "min-h-0 h-full rounded-lg border border-[var(--faded-bg-color)] p-2";
});

const optionsLayoutClass = computed(() => {
  return chartType.value === "bar" ? "min-h-0 flex flex-wrap items-stretch gap-3 overflow-y-auto pr-1" : "min-h-0 flex flex-col gap-2 overflow-y-auto pr-1";
});
